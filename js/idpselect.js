function IdPSelectUI() {
    ...
    */
    var saveUserSelectedIdPs = function(entityIDs){
        ...
        for(var i=length; i > 0; i--){
            var entityID = entityIDs[i-1];
            if (entityID.length > 0) {
                if (idps.filter(function(idp){ return idp.entityID === entityID }).length == 0) {
                    // Grab the IdP object from the JSON data.
                    var idpObj = getIdPFor(entityID);

                    // See if we can get an image src URL from the JSON.
                    var imageSrc = defaultLogo;
                    var image = getImageForIdP(idpObj, false);
                    if (image !== null) {
                        if (image.src !== null) {
                            imageSrc = image.src;
                        }
                    ...
                    // See if we can get text from the JSON. Returns
                    // the entityID if no name is found.
                    var idpText = getLocalizedName(idpObj);
